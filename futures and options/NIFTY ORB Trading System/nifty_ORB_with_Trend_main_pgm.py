...
Undrestanding Breakout Strategy
-------------------------------
...
-------------
Entry Condition:
If all the three conditions are fulfilled, then take a Long Position
	1) 5 minute candlestick Close above the High of the "Open Range"
    2) Today's TR is less than 75% of yesterdays ATR (Calculate 14 day ATR on daily time frame)
    3) The SuperTrend indicator for All the three timeframe's - 5 minute candle, 15 minute candle and 30 minute candle is 'down' (below the Price).

Fixed Stop:
    This is a "False Stop". It is placed at 1 ATR below the Open Range Low.
    To handle emergency situations of sudden high volatility.
    
Trailing Stop:
    During its journey, many times, the 5 minute candlestick will go below the Super Trend but actually Close above the Super Trend. If the Trailing Stop is hardcoded
    to the SuperTrend value, the trailing stop will get hit. This is a false signal which needs to be weeded out (operator driven shitty games !!!).
    So instead of hardcoding the Trailing Stop, we have a False Stop (Fixed Stop) and an exit condition which lay emphasis on the Close of the candle.

Exit Condition:
    5 min Candlestick Close below the 5 Minute SuperTrend indicator (SuperTrend indicator is above the Price) OR the time
    is more than 15:15 hrs Indian Standard Time(IST)
...
def calc_timeframes_trend(client, df_minute):
    '''calculate trend for timeframe 5min, 15min and 30 mins from minute data '''
    df_5_min = df_minute.copy(deep=True)
    df_5_min['Date'] = pd.to_datetime(df_5_min['Date']) # Convert to pandas DateTime format
    df_5_min = SuperTrend(df_5_min, 14, 2, ohlc=['Open', 'High', 'Low', 'Close'])  
    print ('Minute from datetime.now: {}'.format(datetime.datetime.now().minute))
    print('Minute in dataframe last value: {}'.format(df_5_min['Date'].iloc[-1].minute))
    trend_5_min = df_5_min['STX_14_2'].iloc[-1]
        
    df_15_min = df_minute.copy(deep=True)
    df_15_min['Date'] = pd.to_datetime(df_15_min['Date']) # Convert to pandas DateTime format
    df_15_min = df_15_min.resample(rule='15Min', on='Date').agg({'Open':'first', 'High':'max', 'Low':'min', 'Close':'last', 'Volume':'sum'})
    df_15_min = df_15_min.dropna()
    df_15_min = SuperTrend(df_15_min, 14, 2, ohlc=['Open', 'High', 'Low', 'Close'])
    trend_15_min = df_15_min['STX_14_2'].iloc[-1]
    
    df_30_min = df_minute.copy(deep=True)
    df_30_min['Date'] = pd.to_datetime(df_30_min['Date']) # Convert to pandas DateTime format
    df_30_min = df_30_min.resample(rule='30Min', on='Date').agg({'Open':'first', 'High':'max', 'Low':'min', 'Close':'last', 'Volume':'sum'})
    df_30_min = df_30_min.dropna()
    df_30_min = SuperTrend(df_30_min, 14, 2, ohlc=['Open', 'High', 'Low', 'Close'])
    trend_30_min = df_30_min['STX_14_2'].iloc[-1]

    print ('5 Min Trend is {}, 15 Min Trend is {}, 30 Min Trend is {}'.format(trend_5_min, trend_15_min, trend_30_min))
    return trend_5_min, trend_15_min, trend_30_min
...
def close_open_position(client, action:str, quantity:int):
    '''Open Position is diff from Open Order. Close open position '''
...
def close_open_orders(client):
    ''' Stop Orders are open orders. Close them. Only then exit the market'''
...
def main():
    ...
    # while the time is below 15:30 hrs.
    while ((datetime.datetime.now().time() <= market_close_time)):
        if ((datetime.datetime.now().minute % interval_minutes == 0) and (datetime.datetime.now().second == 0)):
            ...
            # Get the trend from diff timeframes
            trend_5_min, trend_15_min, trend_30_min = calc_timeframes_trend(client, df_minute)            
            ...
            if order_placed is False:
                if datetime.datetime.now().time() >= my_latest_entry_time:
                    ...
                    if df_minute['Close'].iloc[-1] > todays_OR_high:
                        if trend_5_min == 'up' and trend_15_min == 'up' and trend_30_min == 'up':
                            action = 'BUY'
                            ...
                            print("Timeframe Trends not Unanimous. Dont Enter Long position")

                    # For a Short position
                    if df_minute['Close'].iloc[-1] < todays_OR_low:
                        if trend_5_min == 'down' and trend_15_min == 'down' and trend_30_min == 'down':
                            action = 'SELL'
                            ...
                            print("Timeframe Trends not Unanimous. Dont enter Short position")
            ...
            if order_placed is True:
                ...
                if client.local_symbol in client.position_dict:
                    ...
                    if client.position_dict[client.local_symbol] > 0: # Long has been initially taken, after close above OR High
                        if ((trend_5_min == 'down') or (datetime.datetime.now().time() >= my_close_time)):
                            print ('Long has been initially taken. trend now is down OR time > 3:15. Now place a short to Exit position')
                        ...
                        if ((trend_5_min == 'up') or (datetime.datetime.now().time() >= my_close_time)):
                            print ('Short has been initially taken. trend now is up OR time > 3:15. Now place a Long to Exit position')
